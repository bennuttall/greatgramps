...
def _make_shared_ctx(config, db):
    ...
    for _gid, _pdata in all_people.items():
        ...
        if _p.get_privacy() and not config.include_private:
...
def _render_person_pages(ctx, gid, relation, by_marriage, marriage_relation=None):
    ...
    all_pictures = [] if is_private_subject else [
        ...
        for pic in get_all_person_pictures(db, p, config.db_path, include_private=config.include_private)
    ]
    occupations = [] if is_private_subject else get_occupations(p)
    notes = [] if is_private_subject else get_person_notes(db, p)
    attributes = [] if is_private_subject else get_person_attributes(p)
...
def _render_event_page(ctx, slug, event_data, relation_map):
    ...
    if template == 'marriage':
        for pd in filter(None, event_data.get('couple') or []):
            ...
            show_p = p_obj and not (p_obj.get_privacy() and not config.include_private)
            father_obj, mother_obj = get_parents(db, p_obj) if show_p else (None, None)
            person_photos = [] if not show_p else [
                {**photo, 'url': process_photo(photo, media_dir, pd['gramps_id'], config.site_root)}
                for photo in get_photos(db, p_obj, config.db_path, include_private=config.include_private)
            ]
        ...
        if 'events' in named:
            ...
            for event_id in event_ids:
                if event_id not in by_gramps_id:
                    event_obj = db.get_event_from_gramps_id(event_id)
                    if event_obj and event_obj.get_privacy() and not config.include_private:
                        print(f'Event {event_id!r} is private — skipped (set include_private in config to build it)')
                    else:
